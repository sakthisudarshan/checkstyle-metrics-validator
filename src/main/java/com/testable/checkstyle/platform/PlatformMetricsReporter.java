...
public final class PlatformMetricsReporter {

    private final MetricsEngine engine = new MetricsEngine();

    private PlatformMetricsReporter() {
    }

    public static PlatformScanReport fromAudit(AuditSummary summary) {
        return new PlatformMetricsReporter().compute(summary);
    }

    public PlatformScanReport compute(AuditSummary summary) {
        List<PlatformMetricResult> metrics = new ArrayList<>();
        boolean allPassed = true;

        for (MetricResult result : engine.evaluateAll(summary)) {
            if (!result.isPassed()) {
                allPassed = false;
            }
            metrics.add(new PlatformMetricResult(
